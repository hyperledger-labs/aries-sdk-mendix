 ...
 * @param {boolean} config_filterByNonRevocationRequirements - default true
 * @returns {Promise.<string>}
 */
export async function jsa_aries_agent_proofs_getRequestedCredentialsForProofRequest_deprecated(agent_key, proofRecordId, config_filterByPresentationPreview, config_filterByNonRevocationRequirements) {
	// BEGIN USER CODE
	try{
		...
		if(config_filterByPresentationPreview==null)/*optional*/;
		if(config_filterByNonRevocationRequirements==null)/*optional*/;
		let agent=support.cache.get(agent_key);
		if(agent==null)return Promise.reject("Agent not found in cache");
    let options={};
    if(proofRecordId!=null)options.proofRecordId=proofRecordId;
    if(config_filterByPresentationPreview!=null||config_filterByNonRevocationRequirements!=null)options.config={};
    if(config_filterByPresentationPreview!=null)options.config.filterByPresentationPreview=config_filterByPresentationPreview;
    if(config_filterByNonRevocationRequirements!=null)options.config.filterByNonRevocationRequirements=config_filterByNonRevocationRequirements;
    console.info(JSON.stringify(options));
    let ret=await agent.proofs/*.getService()*/.getRequestedCredentialsForProofRequest(options);
